...
let audioCtx=null, masterGain=null, ambientNodes=[], ambienceAudio=null, scoreAudio=null, scoreKey=null, objectCleanup=null, lastFocus=null, cameraAnim=null, toneAnim=null, focusPushAnim=null, focusDismiss=null, lastFlowerKey=null, shotBusy=false;
const el={
...
function runSceneDirector(key){
 if(cameraAnim){try{cameraAnim.cancel()}catch{}cameraAnim=null}if(toneAnim){try{toneAnim.cancel()}catch{}toneAnim=null}
 const cfg=CAMERA_TRACKS[key]||CAMERA_TRACKS.prologue;
 if(!isReducedMotion()&&el.sceneArt.animate){cameraAnim=el.sceneArt.animate(cfg.frames,{duration:cfg.duration,easing:'cubic-bezier(.36,.02,.2,1)',fill:'forwards',direction:'alternate',iterations:Infinity});toneAnim=el.tone.animate([{opacity:.72},{opacity:.96},{opacity:.78}],{duration:cfg.duration*.72,easing:'ease-in-out',direction:'alternate',iterations:Infinity})}
...
$('#motionCheck').onchange=e=>{state.settings.reduceMotion=e.target.checked;document.documentElement.classList.toggle('reduce-motion',state.settings.reduceMotion);if(isReducedMotion()){try{cameraAnim?.cancel()}catch{}try{toneAnim?.cancel()}catch{}cameraAnim=toneAnim=null;el.sceneArt.style.transform=''}else runSceneDirector(state.scene);seedPetals();save()};$('#resetBtn').onclick=()=>{if(confirm('确认清除《花语》的本机存档？'))reset()};
...
function openFocusScene(label,sourceButton){return new Promise(async resolve=>{
 const r=sourceButton?.getBoundingClientRect?.(),x=r?(r.left+r.width/2):innerWidth/2,y=r?(r.top+r.height/2):innerHeight/2;

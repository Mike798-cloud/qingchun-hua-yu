...
const PETAL_FILES={daisy:[1,2,3,4,5].map(i=>`assets/petals/daisy_${i}.webp`),sunflower:[1,2,3,4,5].map(i=>`assets/petals/sunflower_${i}.webp`),gardenia:[1,2,3,4,5].map(i=>`assets/petals/gardenia_${i}.webp`),jacaranda:[1,2,3,4,5].map(i=>`assets/petals/jacaranda_${i}.webp`),forget:[1,2,3,4,5].map(i=>`assets/petals/forget_${i}.webp`)};
...
async function ending(){setCheckpoint('ending');renderScene('road',{cinematic:true});scoreKey=null;playScoreForScene('road');stopAmbient();playSfx('bell');await playMemoryMontage();await playCinematicShot('empty',3200);const endLines=state.flags.replay?[{t:'我知道下一句是什么。'},{t:'可到这里，我还是不想往后点。'},{t:'那时候总说“明天再讲”。有些话就差了这一天。'}]:[{t:'高中三年，过的时候慢得要命。'},{t:'很多话都拖到“明天再说”。'},{t:'然后毕业了。'}];await say(endLines);openObject(`<div class="ending-card"><p class="big">毕业那天，我们都说有空再见。</p><p>那时候真觉得很容易。</p><p>后来当然也见过面，只是再没有一次——</p><p class="big">下课铃一响，四个人刚好都在同一条走廊。</p><h2>花语</h2><button class="paper-btn" id="endBack">回到封面</button></div>`,()=>{$('#endBack').onclick=()=>{state.completed=true;state.checkpoint='complete';try{localStorage.setItem(META,'1')}catch{}save();closeObject(true);el.game.hidden=true;el.title.classList.add('active')}},false)}

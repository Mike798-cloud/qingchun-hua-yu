...
let audioCtx=null, masterGain=null, ambientNodes=[], ambienceAudio=null, scoreAudio=null, scoreKey=null, objectCleanup=null, lastFocus=null, cameraAnim=null, hotspotCameraAnim=null, toneAnim=null, focusPushAnim=null, focusDismiss=null, lastFlowerKey=null, shotBusy=false, supportPromptTimer=0, endingPetalStage=0;
const petalPointer={x:-9999,y:-9999,active:false,last:0};
const el={
  ...
  object:$('#objectOverlay'),objectContent:$('#objectContent'),side:$('#sidePanel'),sideContent:$('#sideContent'),settings:$('#settingsPanel'),memory:$('#memoryVignette'),montage:$('#montageScene'),montageImage:$('#montageImage'),focus:$('#focusScene'),focusFrame:$('#focusFrame'),focusVisual:$('#focusVisual'),tone:$('#sceneTone'),shade:$('#cameraShade'),sceneFx:$('#sceneFx'),actors:$('#sceneActors'),flower:$('#flowerLabel'),cinematic:$('#cinematicShot'),cinematicImage:$('#cinematicShotImage'),ending:$('#endingSequence'),endingWords:$('#endingWords'),endingLogo:$('#endingLogo'),endingBack:$('#endingBack')
};
...
const SCENE_ACTOR_PROFILES={
 ch1class:['light'],cafeteria:['light'],sunset:['light'],night:[],library:['light'],festival:['light'],radio:[],high3:['light'],plant:['light'],gradcg:['light'],gate:['light'],road:[]
};
function configureSceneActors(key){if(!el.actors)return;const a=SCENE_ACTOR_PROFILES[key]||[];el.actors.innerHTML=a.includes('light')?'<i class="actor-lightband"></i>':''}
const CINEMATIC_SHOTS={
 corridor:['assets/art/shots/corridor_spring.webp','春日下午的教学楼走廊'],
 cafeteria:['assets/art/shots/cafeteria_shared.webp','四名学生在食堂分一盒鸡米花'],
 window:['assets/art/shots/window_evening.webp','傍晚教室靠窗的两名学生'],
 festival:['assets/art/shots/festival_backstage.webp','校园艺术节后台与相机'],
 countdown:['assets/art/shots/countdown_48.webp','高三教室黑板上的毕业倒计时'],
 bellflower:['assets/art/shots/bellflower_hands.webp','毕业日藏在身后的桔梗'],
 empty:['assets/art/shots/empty_classroom.webp','毕业后的空教室'],
 walk:['assets/art/shots/walk_home.webp','放学后的校园道路'],
 zhixiaIntro:['assets/art/focus/zhixia-desk.webp','沈知夏在教室里的第一印象'],
 zhouyeIntro:['assets/art/focus/zhouye.webp','周野在教室里的第一印象']
};
const imageCache=new Map();
function preloadImage(src){
 if(!src)return Promise.resolve(null);
 if(imageCache.has(src))return imageCache.get(src);
 const task=new Promise(resolve=>{const im=new Image();im.decoding='async';im.src=src;const done=()=>{try{const d=im.decode?.();if(d&&typeof d.then==='function')d.then(()=>resolve(im)).catch(()=>resolve(im));else resolve(im)}catch{resolve(im)}};if(im.complete)done();else{im.addEventListener('load',done,{once:true});im.addEventListener('error',()=>resolve(null),{once:true})}});
 imageCache.set(src,task);return task
}
function preloadSceneBundle(key){const sc=scenes[key];if(sc)preloadImage(sc.art);Object.values(CINEMATIC_SHOTS).forEach(([src])=>preloadImage(src))}
async function playCinematicShot(key,ms=2600){
 if(shotBusy||!el.cinematic)return;const cfg=CINEMATIC_SHOTS[key];if(!cfg)return;shotBusy=true;
 const [src,alt]=cfg;await preloadImage(src);el.cinematicImage.src=src;el.cinematicImage.alt=alt;el.cinematic.hidden=false;playSfx('transition');
 requestAnimationFrame(()=>requestAnimationFrame(()=>el.cinematic.classList.add('show')));
 await sleep(isReducedMotion()?220:ms);el.cinematic.classList.remove('show');await sleep(isReducedMotion()?50:360);el.cinematic.hidden=true;shotBusy=false
}
...
function finishTyping(){if(!typingState)return;cancelAnimationFrame(typingRaf);typingRaf=0;el.line.textContent=typingState.full;el.line.classList.remove('typing');typingState=null}
function lineSpeaker(d){if(d.s)return d.s;if(/^第[一二三四五六七八九十]+章/.test(d.t))return '';return state.chapter===0&&state.scene==='prologue'?'林屿':'林屿 · 心里'}
function showNextLine(){
 if(typingState){finishTyping();return}
 const d=dialogueQueue.shift();
 if(!d){el.dialogue.hidden=true;el.dialogue.dataset.thought='0';el.line.classList.remove('typing');el.speaker.textContent='';duckMix(false);const r=dialogueResolve;dialogueResolve=null;r&&r();return}
 duckMix(true);el.dialogue.dataset.thought=d.s?'0':'1';el.speaker.textContent=lineSpeaker(d);el.line.textContent='';el.line.dataset.full=d.t;el.line.classList.add('typing');
 const chars=[...d.t],base=Math.max(14,34/state.settings.textSpeed),schedule=[];let total=0;
 for(const ch of chars){total+=base*(/[。！？]/.test(ch)?4.8:/[，、；：]/.test(ch)?2.2:/[…—]/.test(ch)?3.2:/\n/.test(ch)?3.4:1);schedule.push(total)}
 typingState={full:d.t,chars,schedule,start:0,index:0};
 const step=ts=>{if(!typingState)return;if(!typingState.start)typingState.start=ts;const elapsed=ts-typingState.start;let i=typingState.index;while(i<schedule.length&&schedule[i]<=elapsed)i++;if(i!==typingState.index){typingState.index=i;el.line.textContent=chars.slice(0,i).join('')}if(i>=chars.length){el.line.textContent=d.t;el.line.classList.remove('typing');typingState=null;typingRaf=0;return}typingRaf=requestAnimationFrame(step)};
...
function rampAudio(a,target,ms=320){if(!a)return;cancelAnimationFrame(a._huayuRampRaf||0);const start=a.volume,t0=performance.now();const step=t=>{const q=Math.min(1,(t-t0)/Math.max(1,ms)),e=1-Math.pow(1-q,3);a.volume=Math.max(0,Math.min(1,start+(target-start)*e));if(q<1)a._huayuRampRaf=requestAnimationFrame(step);else a._huayuRampRaf=0};a._huayuRampRaf=requestAnimationFrame(step)}
...
function stopAmbient(){ambientNodes.forEach(n=>{try{n.stop?.()}catch{}try{n.disconnect?.()}catch{}});ambientNodes=[];if(ambienceAudio){const a=ambienceAudio;ambienceAudio=null;rampAudio(a,0,220);setTimeout(()=>{try{a.pause();a.removeAttribute('src');a.load?.()}catch{}},240)}}
...
function playScoreForScene(key){const k=scoreForScene(key);if(scoreKey===k&&scoreAudio&&!scoreAudio.paused)return;scoreKey=k;if(scoreAudio){const old=scoreAudio;scoreAudio=null;rampAudio(old,0,260);setTimeout(()=>{try{old.pause();old.removeAttribute('src');old.load?.()}catch{}},290)}const file=AUDIO.score[k];if(!file)return;try{const a=new Audio('assets/audio/'+file);a.loop=true;a.preload='auto';a.volume=0;scoreAudio=a;a.play().then(()=>rampAudio(a,Math.min(.52,state.settings.music*.48),440)).catch(()=>{})}catch{}}
...
function focusMarkup(label){const file=focusAssets[label]||'generic.webp',kind=focusKind(label);return `<figure class="focus-art-wrap" data-kind="${kind}"><img class="focus-art" src="assets/art/focus/${file}" alt="${label}的独立重构特写"></figure>`}
function playFocusCue(kind){const cue=(kind==='paper'||kind==='board')?'focusPaper':kind==='window'?'focusWindow':kind==='person'?'focusPerson':(kind==='plant'||kind==='flower')?'focusFlower':'focus';playSfx(cue)}
...
function focusPushIn(sourceButton){
 ...
 focusPushAnim=el.sceneArt.animate([{transform:getComputedStyle(el.sceneArt).transform==='none'?'scale(1.02)':getComputedStyle(el.sceneArt).transform},{transform:`translate3d(${dx}px,${dy}px,0) scale(1.17)`}],{duration:460,easing:'cubic-bezier(.2,.72,.12,1)',fill:'forwards'});
 return focusPushAnim.finished.catch(()=>{});
}
function focusPullOut(){try{focusPushAnim?.cancel()}catch{}focusPushAnim=null;el.sceneArt.style.transform='';el.shade.classList.remove('focus-push');el.hotspots.style.pointerEvents='';el.hotspots.classList.remove('focus-transition');runSceneDirector(state.scene)}
function openFocusScene(label,sourceButton){return new Promise(async resolve=>{
 const pt=hotspotPoint(sourceButton),x=pt.x,y=pt.y,file=focusAssets[label]||'generic.webp',src='assets/art/focus/'+file;
 el.shade.style.setProperty('--focus-x',x+'px');el.shade.style.setProperty('--focus-y',y+'px');await preloadImage(src);await focusPushIn(sourceButton);duckMix(true);
 el.focus.style.setProperty('--focus-x',x+'px');el.focus.style.setProperty('--focus-y',y+'px');el.focus.dataset.kind=focusKind(label);el.focusVisual.innerHTML=focusMarkup(label);el.focus.hidden=false;el.focus.classList.remove('ready','leaving');el.focus.classList.add('entering');playFocusCue(focusKind(label));
 ...
 let done=false;const finish=(go)=>{if(done)return;done=true;focusDismiss=null;el.focus.classList.add('leaving');el.focus.classList.remove('ready');setTimeout(()=>{el.focus.hidden=true;el.focus.classList.remove('leaving','entering');$('#focusInspect').onclick=null;$('#focusBack').onclick=null;focusPullOut();duckMix(false);sourceButton?.focus?.({preventScroll:true});resolve(go)},isReducedMotion()?40:300)};
 focusDismiss=()=>finish(false);$('#focusInspect').onclick=()=>finish(true);$('#focusBack').onclick=()=>finish(false);setTimeout(()=>$('#focusInspect')?.focus({preventScroll:true}),isReducedMotion()?40:520)
})}
...
async function playMemoryMontage(){
 const personal=memoryShotPool().slice(0,7).map(([src])=>src);
 const fixed=['assets/art/group_photo.webp','assets/art/shots/corridor_spring.webp','assets/art/shots/cafeteria_shared.webp','assets/art/shots/window_evening.webp','assets/art/shots/festival_backstage.webp','assets/art/cg_peace.webp','assets/art/shots/countdown_48.webp','assets/art/shots/bellflower_hands.webp','assets/art/cg_graduation.webp','assets/art/cg_pressflower.webp','assets/art/shots/empty_classroom.webp'];
 const shots=[...personal,...fixed];await Promise.all(shots.slice(0,4).map(preloadImage));el.montage.hidden=false;el.montage.classList.remove('cut','out','flash','fast');
 for(let i=0;i<shots.length;i++){const src=shots[i];preloadImage(shots[i+1]);el.montage.classList.toggle('fast',i>Math.floor(shots.length*.68));el.montage.classList.remove('cut','out');el.montageImage.src=src;el.montageImage.alt='青春回忆 '+(i+1);if(i>0){el.montage.classList.remove('flash');void el.montage.offsetWidth;el.montage.classList.add('flash')}await sleep(isReducedMotion()?80:110);el.montage.classList.add('cut');await sleep(isReducedMotion()?170:Math.max(520,1320-i*48));el.montage.classList.add('out');await sleep(isReducedMotion()?70:140)}
 el.montage.classList.remove('cut','fast');await sleep(isReducedMotion()?60:520);el.montage.hidden=true
}
...
function setHotspots(items){
 ...
 items.forEach(h=>{
  ...
  preloadImage('assets/art/focus/'+(focusAssets[h.label]||'generic.webp'));
...
// PETALS — pre-rendered hand-painted sprites, gravity first and wind second.
const cv=$('#petals'),ctx=cv.getContext('2d',{alpha:true});let petals=[],petalLastFrame=0;
const PETAL_FILES={daisy:[1,2,3,4,5].map(i=>`assets/petals/daisy_${i}.webp`),sunflower:[1,2,3,4,5].map(i=>`assets/petals/sunflower_${i}.webp`),gardenia:[1,2,3,4,5].map(i=>`assets/petals/gardenia_${i}.webp`),jacaranda:[1,2,3,4,5].map(i=>`assets/petals/jacaranda_${i}.webp`),forget:[1,2,3,4,5].map(i=>`assets/petals/forget_${i}.webp`),bellflower:[1,2,3,4,5].map(i=>`assets/petals/bellflower_${i}.webp`)};
const PETAL_IMAGES={};Object.entries(PETAL_FILES).forEach(([k,arr])=>{PETAL_IMAGES[k]=arr.map(src=>{const im=new Image();im.decoding='async';im.src=src;return im})});
const PETAL_PROFILE={daisy:{fall:.84,sway:.62,tumble:.72,flip:.82,alpha:.78,scale:1.0},sunflower:{fall:1.08,sway:.50,tumble:1.08,flip:.66,alpha:.82,scale:1.08},gardenia:{fall:.68,sway:.44,tumble:.52,flip:.52,alpha:.76,scale:1.12},jacaranda:{fall:1.18,sway:.82,tumble:.92,flip:.94,alpha:.80,scale:.94},forget:{fall:.58,sway:.58,tumble:.46,flip:.58,alpha:.72,scale:.78},bellflower:{fall:.72,sway:.68,tumble:.64,flip:.78,alpha:.79,scale:1.04}};
function resize(){const dpr=Math.min(devicePixelRatio||1,innerWidth<760?1.5:2);cv.width=Math.round(innerWidth*dpr);cv.height=Math.round(innerHeight*dpr);cv.style.width=innerWidth+'px';cv.style.height=innerHeight+'px';ctx.setTransform(dpr,0,0,dpr,0,0);seedPetals()}
addEventListener('resize',resize,{passive:true});
function pick(arr){return arr[(Math.random()*arr.length)|0]}
function petalProfile(type){return PETAL_PROFILE[type]||PETAL_PROFILE.daisy}
function activePetalTypes(f){
 // Ending returns the chapter flowers one after another; bellflower owns the last beat.
 if(endingPetalStage===1)return ['daisy'];
 if(endingPetalStage===2)return ['sunflower'];
 if(endingPetalStage===3)return ['gardenia'];
 if(endingPetalStage===4)return ['jacaranda'];
 if(endingPetalStage===5)return ['forget'];
 if(endingPetalStage===6)return ['bellflower','bellflower','bellflower','forget'];
 if(endingPetalStage>=7)return ['bellflower'];
 return f.key==='mixed'?['daisy','sunflower','gardenia','jacaranda','forget']:[f.key]
}
function spawnPetal(p={},fresh=false){const f=flowerNow(),mobile=innerWidth<760,types=activePetalTypes(f),type=pick(types),cfg=petalProfile(type);p.type=type;p.sprite=(Math.random()*5)|0;p.depth=.48+Math.random()*1.34;p.baseSize=(f.size[0]+Math.random()*(f.size[1]-f.size[0]))*(mobile?.88:1)*(type==='bellflower'?1.18:1);p.v=(endingPetalStage?Math.max(.16,f.speed*.86):f.speed)*(.62+Math.random()*.92);p.wind=(endingPetalStage?Math.max(.14,f.wind*.9):f.wind)*(.48+Math.random()*.76);p.angle=Math.random()*Math.PI*2;p.spin=(Math.random()-.5)*(.009+.012*Math.random());p.phase=Math.random()*Math.PI*2;p.phase2=Math.random()*Math.PI*2;p.x=fresh?Math.random()*innerWidth:Math.random()*(innerWidth+60)-30;p.y=fresh?Math.random()*innerHeight:-36-Math.random()*innerHeight*.55;p.cfg=cfg;return p}
function seedPetals(){const f=flowerNow(),mobile=innerWidth<760;let n;if(endingPetalStage){const sparse=endingPetalStage>=6;n=mobile?(sparse?12:17):(sparse?20:27)}else n=Math.round(f.density*(mobile?.68:.96));n=Math.max(mobile?12:18,Math.min(mobile?35:60,n));petals=Array.from({length:n},()=>spawnPetal({},true))}
function drawPetal(p,t){const cfg=p.cfg||petalProfile(p.type),imgs=PETAL_IMAGES[p.type]||PETAL_IMAGES.daisy,im=imgs[p.sprite%imgs.length];if(!im||!im.complete||!im.naturalWidth)return;const size=p.baseSize*(3.7+p.depth*1.65)*cfg.scale,flip=Math.max(.16,Math.abs(Math.cos(p.phase2))*cfg.flip+.12),near=p.depth>1.35;ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.angle);ctx.scale(1,flip);ctx.globalAlpha=Math.min(.9,cfg.alpha*(.62+p.depth*.22));if(near)ctx.filter='blur(.8px)';ctx.drawImage(im,-size/2,-size/2,size,size);ctx.restore()}
addEventListener('pointermove',e=>{petalPointer.x=e.clientX;petalPointer.y=e.clientY;petalPointer.active=true;petalPointer.last=performance.now()},{passive:true});
addEventListener('pointerleave',()=>{petalPointer.active=false},{passive:true});
resize();
(function drawPetals(ts=0){requestAnimationFrame(drawPetals);if(document.hidden||isReducedMotion())return;const mobile=innerWidth<760,frameMs=1000/(mobile?34:52);if(ts-petalLastFrame<frameMs)return;const dt=Math.min(2.2,(ts-petalLastFrame)/16.67||1);petalLastFrame=ts;ctx.clearRect(0,0,innerWidth,innerHeight);const overlayBusy=(!el.focus.hidden||!el.cinematic.hidden||!el.montage.hidden),stride=overlayBusy?2:1,globalWind=Math.sin(ts*.00048)*.16+Math.sin(ts*.00019)*.11;for(let i=0;i<petals.length;i+=stride){const p=petals[i],cfg=p.cfg;p.phase+=.018*dt*(.82+cfg.sway*.3);p.phase2+=.025*dt*(.72+cfg.flip*.32);p.y+=p.v*p.depth*cfg.fall*dt*(1+.07*Math.sin(p.phase*.55));p.x+=(Math.sin(p.phase)*p.wind*cfg.sway+Math.sin(p.phase*.43)*p.wind*.32+globalWind)*p.depth*dt;p.angle+=p.spin*cfg.tumble*dt+Math.sin(p.phase2)*.0018;p.x+=Math.sin(p.phase2*1.65)*.12*p.depth*dt;if(petalPointer.active&&ts-petalPointer.last<850){const dx=p.x-petalPointer.x,dy=p.y-petalPointer.y,d2=dx*dx+dy*dy;if(d2<19000&&d2>12){const force=(1-d2/19000)*.65*p.depth;p.x+=dx/Math.sqrt(d2)*force*dt;p.y+=dy/Math.sqrt(d2)*force*.22*dt}}if(p.y>innerHeight+50||p.x<-70||p.x>innerWidth+70)spawnPetal(p,false);drawPetal(p,ts)}})();
...
async function transitionToPast(){setCheckpoint('transitionPast');renderScene('photo',{memory:true,cinematic:true});await say([{t:'照片边上都糊了。周野那半张脸还是晃的。'},{s:'周野',t:'快点快点，再拍一张！'},{s:'唐梨',t:'你已经拍十六张了。'},{s:'沈知夏',t:'林屿你不要每次都闭眼。'}]);await playCinematicShot('corridor',2500);state.chapter=1;setCheckpoint('ch1class');renderScene('ch1class',{memory:true});ambient('room');await say([{t:'第一章 · 向日葵｜鸡米花应该算一道菜吗'},{s:'老师',t:'四人一组。沈知夏记录，唐梨拍照，林屿整理文字。'}]);await playCinematicShot('zhixiaIntro',1050);await say([{s:'沈知夏',t:'好。'}]);await playCinematicShot('zhouyeIntro',900);await say([{s:'周野',t:'我可以负责战略统筹。'},{s:'老师',t:'你负责搬花盆。'},{s:'唐梨',t:'……挺适合你的。'}]);setupCh1Class();}
function enableFirstDragTutorial(){
 if(state.dialogueFlags?.dragTutorialSeen)return ()=>{};
 document.body.classList.add('drag-onboarding');let firstDropSucceeded=false,armed=false;
 const failOnce=()=>{if(firstDropSucceeded||state.dialogueFlags.dragTutorialSeen)return;state.dialogueFlags.dragTutorialSeen=true;document.body.classList.remove('drag-onboarding');toast('可以拖动它。');save()};
 const success=()=>{firstDropSucceeded=true;if(!state.dialogueFlags.dragTutorialSeen){state.dialogueFlags.dragTutorialSeen=true;save()}document.body.classList.remove('drag-onboarding')};
 return {failOnce,success,arm:()=>{armed=true},isArmed:()=>armed}
}
function puzzle2(){
 ...
 openObject(`<div class="physical plant-board"><div class="plant-visual"><svg viewBox="0 0 520 440" aria-label="植物观察对象"><rect width="520" height="440" fill="#bcc9bd"/><rect y="350" width="520" height="90" fill="#9b8569"/><path d="M260 360 C250 300 270 250 258 175 C245 120 270 75 250 38" stroke="#53745a" stroke-width="12" fill="none"/><path d="M254 210 C200 180 170 180 132 145 C195 145 238 165 260 195 M260 275 C315 245 350 230 388 195 C330 195 290 215 260 250" stroke="#628164" stroke-width="15" fill="none"/><path d="M250 45 c-45 -35 -85 10 -62 52 c-49 7 -44 64 2 73 c-17 47 40 70 68 31 c34 35 84 4 69 -38 c49 -10 45 -65 -3 -74 c12 -46 -39 -74 -74 -44z" fill="#b6a565" stroke="#625c42" stroke-width="7"/><rect x="190" y="330" width="140" height="70" fill="#8c6f55" stroke="#5e4939" stroke-width="6"/><rect x="225" y="357" width="70" height="28" fill="#d9ccb1"/><text x="238" y="378" font-size="18" fill="#5a5144">B-17</text><path d="M380 60 L480 20" stroke="#e6d5a1" stroke-width="35" opacity=".32"/></svg>${pts.map(([n,x,y])=>`<button class="plant-point" data-name="${n}" style="left:${x}%;top:${y}%" aria-label="观察${n}"></button>`).join('')}</div><div class="plant-notes"><h2>植物观察</h2><p id="observeCount">已观察 0 / 5</p><div class="slots"><div class="slot" data-answer="5枚花瓣">花瓣：<span></span></div><div class="slot" data-answer="披针形">叶片：<span></span></div><div class="slot" data-answer="直立">茎：<span></span></div><div class="slot" data-answer="右上方">光照：<span></span></div><div class="slot" data-answer="B-17">标签：<span></span></div><div class="slot" data-answer="6月12日">日期：<span></span></div></div><div class="chips">${['右上方','B-17','披针形','5枚花瓣','直立','6月12日'].map(x=>`<div class="chip" draggable="true">${x}</div>`).join('')}</div><button class="puzzle-btn" id="finishPlant">完成第一页</button><div id="plantStatus" class="status-line"></div></div></div>`,()=>{
  ...
  let drag='',dropOk=false;const dragGuide=enableFirstDragTutorial();$$('.chip').forEach(c=>{c.ondragstart=()=>{drag=c.textContent;dropOk=false;c.classList.add('dragging');dragGuide.arm?.()};c.ondragend=()=>{c.classList.remove('dragging');if(!dropOk)dragGuide.failOnce?.()};c.onclick=()=>{drag=c.textContent;toast('已拿起：'+drag)}});$$('.slot').forEach(s=>{s.ondragover=e=>e.preventDefault();const put=()=>{if(!drag)return;s.querySelector('span').textContent=drag;s.dataset.value=drag;s.classList.add('filled');drag='';dropOk=true;dragGuide.success?.()};s.ondrop=e=>{e.preventDefault();put()};s.onclick=put});
...
function puzzle6(){
 ...
 openObject(`<div class="physical radio-console"><h2>广播磁带</h2><p>这次磁带真的可以听。节目从17:40开始，后台话筒被误录进了其中一个声道。</p><div class="radio-display" id="radioTime">17:40:00</div><div class="tape-window"><i></i><i></i><span>校园广播留档 · A面</span></div><div id="tapeWave" class="tape-waveform">${bars}</div><input class="tape-slider" id="tape" type="range" min="0" max="220" step="1" value="0" aria-label="磁带位置"><div class="transport"><button class="puzzle-btn" id="playTape">播放</button><button class="puzzle-btn" data-d="-15">倒带 15s</button><button class="puzzle-btn" data-d="15">快进 15s</button></div><div class="channel-row"><button class="channel" data-ch="L">左声道</button><button class="channel active" data-ch="R">右声道</button></div><div class="transcript" id="transcript">右声道是一段有完整旋律和节拍的校园点歌节目。</div><p class="radio-ear-note">建议戴耳机。左/右声道是两条真实音轨；切换时会保留当前时间。</p><button class="puzzle-btn" id="confirmTape">确认这段录音</button><div id="radioStatus" class="status-line"></div></div>`,()=>{
  ...
  $('#confirmTape').onclick=async()=>{const v=+(active().currentTime||tape.value);if(ch!=='L'||Math.abs(v-195)>7){fails++;$('#radioStatus').textContent=fails>=2?'完整定位：左声道，17:43:15 左右。那里能听到节目旋律下面突然出现压低的人声。':'还不是。节目17:40开始；那句话在开始后大约3分15秒，而且不在节目声道。';return}stop();$('#transcript').innerHTML='唐梨：“那你问他。”<br>沈知夏：“不问。”<br>唐梨：“为什么？”<br><br>沈知夏：“问了以后……可能就连这样都没了。”';state.puzzles.pz6=true;setFlag('confirmedZhixiaLikesLinyu');addItem('广播录音时间');setCheckpoint('tapeReveal');await sleep(500);closeObject();await tapeRevealSequence()};render();return()=>{stop();Object.values(tracks).forEach(a=>{a.src='';a.load?.()})};
 });
}
async function tapeRevealSequence(){setCheckpoint('tapeReveal');renderScene('radio',{memory:true});ambient('room');await say([{s:'唐梨',t:'那你问他。'},{s:'沈知夏',t:'不问。'},{s:'唐梨',t:'为什么？'},{s:'沈知夏',t:'问了以后……可能就连这样都没了。'}]);setCheckpoint('postTape');await postTapeSequence()}
async function postTapeSequence(){setCheckpoint('postTape');renderScene('radio',{memory:true});ambient('room');await say([{s:'周野',t:'根据牛顿第三定律……'},{s:'林屿',t:'你最好不要继续。'},{s:'周野',t:'你喜欢她。那她也有可能喜欢你啊。'},{s:'林屿',t:'牛顿会报警。'},{s:'唐梨',t:'他物理37。'},{s:'周野',t:'停止人身攻击。'}]);startChapter4()}
function puzzle7(){
...
async function addressRevealSequence(){setCheckpoint('addressReveal');renderScene('high3',{memory:true});ambient('room');if(!inspected('addressdoc'))markInspected('addressdoc');await say([{t:'周野把纸翻回去，底下露出一张住址变更单。'},{t:'上面写着她爸爸的新工作地点、住址，还有搬家日期。'},{t:'她会照常毕业。搬家是在毕业以后。'}]);setCheckpoint('conflict');await conflictSequence()}
function puzzle8(){
...
async function page33RevealSequence(){setCheckpoint('page33Reveal');renderScene('notebook',{memory:true});ambient('room');await say([{t:'作文最后还有一段，被我涂得几乎看不见。'},{t:'我也不知道以后会去哪。\n毕业以后……要是还能联系就好了。\n如果她愿意。'},{t:'最下面那句涂了好几遍，还是能认出来。'},{t:'“我好像有点喜欢她。”'}]);setCheckpoint('returnPresent');returnToPresent()}
function puzzle9(){
...
// SCENE SETUPS
async function beginPrologue(){state.started=true;state.chapter=0;state.step=0;setCheckpoint('prologue');el.title.classList.remove('active');el.game.hidden=false;renderScene('prologue');el.sceneArt.style.opacity='0';await prologueSoundscape();el.sceneArt.style.opacity='1';await sleep(450);await say([{t:'九年没进这间教室，我先找了自己的座位。'},{t:'门牌还写着高三七班。'},{t:'手碰到门把，我停了一下。'}]);setupPrologue()}
function setupPrologue(){setCheckpoint('prologue');renderScene('prologue');ambient('room');setHotspots([
 {id:'blackboard',label:'黑板',x:5,y:10,w:44,h:32,done:()=>inspected('blackboard'),onClick:()=>say([{t:'黑板上只剩半句话：\n“距高考还有——”'},{t:'以前天天见，懒得抬头。'},{t:'右边还有没擦净的字：2天。'}])},
 {id:'fan',label:'吊扇',x:46,y:1,w:10,h:9,mx:52,my:50,done:()=>inspected('fan'),onClick:()=>say([{t:'吊扇开到第三档，还是晃。'},{t:'周野赌过它高考前一定会掉。'},{t:'它倒是撑到了最后。'}])},
 {id:'water',label:'饮水机',x:89,y:58,w:10,h:15,mx:50,my:54,done:()=>inspected('water'),onClick:()=>say([{t:'咕噜。'},{t:'还是这个声音。'}])},
 {id:'windowseat',label:'靠窗第三排',x:61,y:54,w:18,h:14,mx:50,my:50,done:()=>inspected('windowseat'),onClick:()=>say([{t:'靠窗第三排。不是我的位置。'}])},
 {id:'scratch',label:'课桌底部刻痕',x:65,y:69,w:18,h:13,mx:48,my:52,done:()=>inspected('scratch'),onClick:()=>say([{t:'L.Y'},{t:'旁边刻着：想得美。'}])},
 {id:'duty',label:'值日表',x:50,y:19,w:8,h:17,mx:52,my:48,done:()=>inspected('duty'),onClick:()=>say([{t:'值日表：6月5日。'}])},
 {id:'gradphoto',label:'毕业照片标签',x:58,y:19,w:8,h:17,mx:48,my:48,done:()=>inspected('gradphoto'),onClick:()=>say([{t:'毕业照标签：6月7日。'}])},
...
function setupCh1Class(){setCheckpoint('ch1class');renderScene('ch1class',{memory:true});ambient('room');setHotspots([
 ...
 {id:'teacherdesk',label:'讲台',x:3,y:55,w:22,h:24,mx:50,my:46,done:()=>inspected('teacherdesk'),onClick:()=>say([{t:'讲台边都是粉笔灰。分组名单还夹在底下。'}])},
 {id:'zhixia',label:'沈知夏的桌面',x:27,y:34,w:22,h:36,mx:58,my:62,done:()=>inspected('zhixia'),onClick:()=>say([{t:'记录本摊着。字很小，句尾常常没标点。'}])},
 {id:'zhouye',label:'周野',x:49,y:30,w:20,h:38,mx:52,my:48,done:()=>inspected('zhouye'),onClick:()=>say([{s:'周野',t:'战略统筹正在休息。'},{s:'唐梨',t:'你只是没事干。'}])}
 ])}
function setupCafeteria(){setCheckpoint('cafeteria');renderScene('cafeteria');ambient('cafeteria');say([{s:'沈知夏',t:'我忘带饭卡了。'},{s:'林屿',t:'你怎么比周野还不靠谱。'},{s:'周野',t:'为什么突然攻击我？'}]).then(()=>{const hotspots=[{id:'buyFood',label:hasFlag('boughtFoodForZhixia')?'饭卡机':'窗口与饭卡机',x:4,y:13,w:47,h:33,mx:34,my:63,done:()=>hasFlag('boughtFoodForZhixia'),onClick:async()=>{if(hasFlag('boughtFoodForZhixia')){await say([{t:'饭卡机亮过一次。她桌前多了一份鸡米花。'}]);return}setFlag('boughtFoodForZhixia');playSfx('tray');await say([{t:'滴——绿灯。'},{s:'林屿',t:'两份。'},{s:'沈知夏',t:'我之后还你。'},{s:'周野',t:'请问第三份属于团队经费吗？'},{s:'唐梨',t:'不属于。'}])}},{label:'鸡米花餐盘',x:66,y:50,w:28,h:30,onClick:puzzle3},{id:'bag',label:'沈知夏的书包',x:50,y:58,w:13,h:25,done:()=>inspected('bag'),onClick:async()=>{setFlag('sawZhixiaCard');await say([{t:state.flags.replay?'她那天其实带了。':'侧袋里有一张饭卡。'}])}},{id:'canteenNoise',label:'远处餐盘声',x:7,y:66,w:22,h:18,done:()=>inspected('canteenNoise'),onClick:async()=>{playSfx('tray');await say([{t:'餐盘撞到桌沿。旁边太吵，谁说什么都听不全。'},{s:'周野',t:'这种环境非常适合掩护犯罪。'},{s:'唐梨',t:'你先把嘴里的咽下去。'}])}}];setHotspots(hotspots)})}
...
async function startChapter2(){await playCinematicShot('walk',2600);state.chapter=2;setCheckpoint('chapter2');renderScene('night',{memory:true});ambient('night');await say([{t:'第二章 · 栀子花｜靠窗第三排'},{t:'晚自习灯太白。窗外黑成一片，我还是往那边看了两次。'}]);setHotspots([{id:'windowReflection',label:'窗户倒影',x:59,y:8,w:34,h:48,done:()=>inspected('windowReflection'),onClick:async()=>{await playCinematicShot('window',2800);await say([{t:'她抬头了。'},{t:'我正好也抬头。'},{t:'……低头。'}])}},{id:'tubeHum',label:'白色灯管',x:32,y:2,w:27,h:13,done:()=>inspected('tubeHum'),onClick:async()=>{await say([{t:'这根灯管比旁边那根亮。'},{t:'后排翻了一页卷子。还不下课。'}])}},{label:'校刊室里的作文',x:6,y:50,w:35,h:34,onClick:puzzle4}])}
...
function libraryPuzzle(){
 ...
 openObject(`<div class="physical"><h2>图书馆 · 空间检索</h2><div class="library-slip"><strong>借阅记录</strong><br>书架字母：QK<br>分类号：495.4<br>著者号：Z17</div><p>先看借阅记录，再在对应书架里找对书脊。不是所有“植物”标题都属于同一分类。</p><div class="shelf-puzzle">${books.map((b,i)=>`<button class="book-spine ${i%2?'tall':''}" data-code="${b[0]}"><span>${b[0]}</span><small>${b[1]}</small></button>`).join('')}</div><div id="libraryStatus" class="status-line"></div></div>`,()=>{$$('.book-spine').forEach(b=>b.onclick=async()=>{if(b.dataset.code!=='QK 495.4 / Z17'){$('#libraryStatus').textContent='书架字母或分类号对不上。';return}addItem('桔梗记录');save();closeObject();await say([{t:'桔梗。'},{t:'旁边画得很完整，下面只有一行很淡的字。'},{t:'“最后一种？”'}]);await say([{s:'林屿',t:'有笔吗？'},{t:'她把笔递过来。'},{s:'沈知夏',t:'你右手是什么？'},{t:'我低头看了一眼——我手里明明有笔。'},{s:'周野',t:'……'},{t:'远处先有人笑了。她也笑了。'}]);startChapter3()})})
}
function setupLibrary(){setCheckpoint('library');renderScene('library');ambient('library');setHotspots([{label:'植物类书架',x:48,y:8,w:48,h:79,onClick:libraryPuzzle},{id:'shelf',label:'书架分类牌',x:4,y:8,w:40,h:78,done:()=>inspected('shelf'),onClick:()=>say([{t:'QK：植物学。QL：动物学。R：医药卫生。'},{t:'借阅记录最后一行：495.4 / Z17。'}])}])}
async function startChapter3(){await playCinematicShot('festival',2500);state.chapter=3;setCheckpoint('chapter3');renderScene('festival',{memory:true});ambient('festival');await say([{t:'第三章 · 栀子花｜“没什么”到底算什么'},{s:'沈知夏',t:'挺好的。'},{s:'林屿',t:'什么？'},{s:'沈知夏',t:'没什么。'}]);setHotspots([{id:'festivalDrink',label:'饮料桌',x:12,y:55,w:24,h:22,done:()=>inspected('festivalDrink'),onClick:()=>say([{t:'杯壁都是水。纸上“少冰”改成“少点冰”，又被划掉。'},{s:'唐梨',t:'你写字为什么每次都要改。'},{s:'林屿',t:'看不顺眼。'}])},{id:'festivalPhoto',label:'唐梨的相机',x:51,y:52,w:18,h:27,done:()=>inspected('festivalPhoto'),onClick:async()=>{playSfx('shutter');await say([{s:'唐梨',t:'别动。'},{t:'咔嚓。相机背后贴着她的名字，字很小。'}])}},{id:'festivalZhou',label:'周野的节目单',x:69,y:48,w:20,h:28,done:()=>inspected('festivalZhou'),onClick:()=>say([{t:'节目单边上写满感叹号：\n“压轴！！！别忘了！！！五点半！！！”'},{s:'周野',t:'重点就应该醒目。'}])}]);setAction('去广播站',()=>{setCheckpoint('radio');renderScene('radio');ambient('room');setAction('操作磁带机',puzzle6);puzzle6()})}
async function startChapter4(){state.chapter=4;setCheckpoint('chapter4');renderScene('high3',{memory:true});ambient('room');await playCinematicShot('countdown',2500);await say([{t:'第四章 · 蓝花楹｜48天听起来还挺久'},{t:'黑板：距毕业48天。'}]);setHotspots([{label:'四张无署名留言',x:25,y:54,w:37,h:28,mx:52,my:48,onClick:puzzle7},{id:'handwriting',label:'四个人的旧作业',x:6,y:49,w:18,h:31,mx:48,my:52,done:()=>inspected('handwriting'),onClick:()=>say([{t:'这张作文涂得最狠，是我的。'},{t:'句尾没标点——沈知夏。'},{t:'三个感叹号，不用猜，周野。'},{t:'剩下这张字最小。唐梨。'}])},{id:'addressdoc',label:'住址资料',x:65,y:42,w:28,h:22,done:()=>inspected('addressdoc'),onClick:()=>say([{t:'上面写着她爸爸的新工作地点、住址，还有搬家日期。'},{t:'不是转学。她会照常毕业。搬家是在毕业以后。'}])}])}
async function conflictSequence(){setCheckpoint('conflict');renderScene('high3');ambient('room');await say([{s:'周野',t:'你毕业以后要走？'},{s:'沈知夏',t:'我本来想毕业前说的。'},{s:'周野',t:'毕业前是哪一天？最后一天？'},{s:'唐梨',t:'这是她自己的事情。'},{s:'周野',t:'我知道。\n但我们不是别人。'},{s:'周野',t:'你一直帮她瞒？'},{s:'唐梨',t:'她让我不要说。那我还能怎么办？'},{s:'周野',t:'不知道。\n但我就是觉得不对。'}]);await sleep(350);renderScene('peacecg',{memory:true,cinematic:true});await say([{t:'第二天，谁都没提昨天的事。'},{s:'周野',t:'谈判物资。'}]);await sleep(650);await say([{t:'周野把四盒鸡米花放下。没人笑。'},{t:'沈知夏先拿了一块。我们才跟着伸手。'},{s:'周野',t:'……所以谈判成功？'},{s:'唐梨',t:'闭嘴。'}]);startChapter5()}
async function startChapter5(){state.chapter=5;setCheckpoint('chapter5');renderScene('notebook',{memory:true});ambient('room');addItem('车票');await say([{t:'第五章 · 勿忘我｜毕业之前'},{t:'植物册里夹着搬家清单。'},{s:'林屿',t:'你是不是毕业以后要走？'},{s:'沈知夏',t:'嗯。'},{s:'林屿',t:'什么时候知道的？'},{s:'沈知夏',t:'挺早。'},{s:'林屿',t:'唐梨知道？'},{s:'沈知夏',t:'嗯。'},{s:'林屿',t:'周野呢？'},{s:'沈知夏',t:'后来知道的。'},{s:'林屿',t:'就我不知道。'},{s:'沈知夏',t:'我本来准备……'},{s:'林屿',t:'什么时候？'},{s:'沈知夏',t:'毕业前。'},{s:'林屿',t:'还有几天算毕业前？'},{s:'沈知夏',t:'林屿。'},{s:'沈知夏',t:'也不是非得告诉你。'},{s:'林屿',t:'也是。'}]);setHotspots([{id:'flowerOrder',label:'夹在册后的花店订单',x:23,y:18,w:25,h:55,mx:50,my:54,done:()=>hasFlag('flowerOrder'),onClick:async()=>{setFlag('flowerOrder');addItem('花店订单');await say([{t:'还有取花单：毕业日，桔梗。取花人是沈知夏。'}])}},{id:'blankBell',label:'桔梗页的空白',x:53,y:17,w:26,h:57,mx:58,my:58,done:()=>inspected('blankBell'),onClick:async()=>{await say([{t:'“桔梗”下面该有观察记录。'},{t:'只有笔压过的痕迹。'}])}}]);setAction('检查植物册页码',()=>{if(!hasFlag('flowerOrder')){toast('册子后面似乎还夹着一张纸。');return}puzzle8()})}
async function returnToPresent(){setCheckpoint('returnPresent');renderScene('prologue');ambient('room');await playCinematicShot('empty',2800);await say([{t:'教室突然安静了。'},{t:'我去看靠窗第三排。'},{t:'没人。'},{t:'最后一页还是空着一行。'},{t:'桔梗已经褪成很浅的蓝紫。'}]);state.chapter=6;setCheckpoint('blackboard');renderScene('blackboard');setAction('开始毕业日重构',puzzle9)}
async function graduationReplay(){setCheckpoint('graduationReplay');renderScene('gradcg',{memory:true,cinematic:true});ambient('outdoor');await say([{t:'毕业日。植物角。'},{t:'她把花藏在身后。我口袋里塞着那篇作文。'},{s:'沈知夏',t:'林屿。'},{s:'林屿',t:'嗯。'},{s:'沈知夏',t:'我有件事想跟你说。'},{s:'林屿',t:'你今晚就走？'},{s:'沈知夏',t:'嗯。'},{s:'林屿',t:'几点？'},{s:'沈知夏',t:'六点多。'},{s:'林屿',t:'那我……'}]);await playCinematicShot('bellflower',3200);await say([{s:'林屿',t:'一路顺风。'},{s:'沈知夏',t:'嗯。'}]);renderScene('presscg',{memory:true,cinematic:true});await say([{t:'后来她又回了一趟教室。'},{t:'桔梗夹进植物册。'},{t:'笔停在“桔梗”下面。'},{t:'她没写，把册子合上了。'}]);setCheckpoint('chase');startChase()}
...
async function adultEnding(){setCheckpoint('adult');renderScene('road');ambient('road');await say([{t:'出校门以后，我站在路边，才想起手机还在口袋里。'}]);openObject(`<div class="phone"><div class="phone-screen"><div class="phone-head"><strong>沈知夏</strong><br><small>高中群成员</small></div><div class="msg">植物册找到了。</div><div class="msg">桔梗那页也找到了。</div><div class="typing" id="typing">当年我……</div><button class="send-btn" id="rewrite">删掉，重新写</button><button class="send-btn" id="sendFinal" hidden>发送</button><div id="delivered"></div></div></div>`,()=>{$('#rewrite').onclick=async()=>{$('#typing').textContent='当年我也喜欢你。\n\n不用回答。\n只是这次想把当年没说完的话说完。';$('#rewrite').hidden=true;$('#sendFinal').hidden=false};$('#sendFinal').onclick=async()=>{$('#delivered').innerHTML='<p style="text-align:right;color:#6d746b">已送达</p>';state.endingFlags.sent=true;setCheckpoint('finalRoad');playSfx('click');await sleep(800);closeObject(true);await finalRoad()}},false)}
async function playRoadEchoes(lines){
 const wasHidden=el.dialogue.hidden,nextHidden=el.next.hidden;
 el.dialogue.hidden=false;el.dialogue.classList.add('memory-echo');el.dialogue.dataset.thought='1';el.speaker.textContent='';el.next.hidden=true;duckMix(true);
 for(const t of lines){el.line.classList.remove('typing');el.line.textContent=t;el.dialogue.classList.remove('echo-out');await sleep(isReducedMotion()?120:1180);el.dialogue.classList.add('echo-out');await sleep(isReducedMotion()?40:190)}
 el.dialogue.classList.remove('memory-echo','echo-out');el.line.textContent='';el.speaker.textContent='';el.next.hidden=nextHidden;el.dialogue.hidden=wasHidden;duckMix(false)
}
async function finalRoad(){
 setCheckpoint('finalRoad');renderScene('road');ambient('road');
 await playRoadEchoes(['“两份？”','“成交。”','“你去的话，我也去。”','“老班来了！”','“合理实验损耗。”','“五十岁也可以吃。”']);
 playSfx('phone');
 openObject(`<div class="phone lockscreen-only"><div class="phone-screen"><div class="notification"><strong>沈知夏</strong><br>周野是不是还欠我们……</div></div></div>`,()=>{const timer=setTimeout(()=>{state.endingFlags.notificationSeen=true;setCheckpoint('ending');closeObject(true);ending()},isReducedMotion()?260:2200);return()=>clearTimeout(timer)},false)
}
async function showEndingWords(html,hold=2300){
 el.endingWords.classList.add('out');await sleep(isReducedMotion()?40:260);el.endingWords.innerHTML=html;el.endingWords.classList.remove('out');await sleep(isReducedMotion()?180:hold)
}
async function ending(){
 setCheckpoint('ending');renderScene('road',{cinematic:true});stopAmbient();scoreKey=null;playScoreForScene('road');document.body.classList.add('ending-mode');document.body.classList.remove('ending-black');endingPetalStage=1;seedPetals();
 el.ending.hidden=false;el.ending.classList.remove('black');el.endingLogo.hidden=true;el.endingLogo.classList.remove('show');el.endingBack.hidden=true;el.endingBack.classList.remove('show');el.endingWords.innerHTML='';
 await sleep(isReducedMotion()?100:650);
 await showEndingWords('有些话，迟了就是迟了。',2050);
 endingPetalStage=2;seedPetals();await sleep(isReducedMotion()?30:260);
 await showEndingWords('有些人，也不会永远停在原地等你。',2250);
 endingPetalStage=3;seedPetals();await sleep(isReducedMotion()?30:260);
 await showEndingWords('可人生不是为了追回所有错过。<br>是为了在明白错过以后，<br>不再把下一句话也留给以后。',3300);
 endingPetalStage=4;seedPetals();await sleep(isReducedMotion()?30:300);
 await showEndingWords('<strong>花有四季，人有一生。</strong><br><strong>何其有幸，与你相遇。</strong>',2700);
 endingPetalStage=5;seedPetals();await sleep(isReducedMotion()?30:300);
 await showEndingWords('趁花正开，趁风温柔，趁路脚下，<br>大胆往前走。<br>别怕来不及。',3200);
 endingPetalStage=6;seedPetals();await sleep(isReducedMotion()?40:420);endingPetalStage=7;seedPetals();
 await showEndingWords('<strong>风会记住我们曾怎样走过这一程……</strong>',2600);
 el.endingWords.classList.add('out');await sleep(isReducedMotion()?60:520);el.ending.classList.add('black');document.body.classList.add('ending-black');endingPetalStage=0;
 if(scoreAudio)rampAudio(scoreAudio,0,isReducedMotion()?80:1100);await sleep(isReducedMotion()?80:1150);playSfx('schoolBell');await sleep(isReducedMotion()?120:900);
 el.endingWords.innerHTML='';el.endingLogo.hidden=false;requestAnimationFrame(()=>requestAnimationFrame(()=>el.endingLogo.classList.add('show')));await sleep(isReducedMotion()?180:2100);el.endingBack.hidden=false;requestAnimationFrame(()=>el.endingBack.classList.add('show'));
 el.endingBack.onclick=()=>{state.completed=true;state.checkpoint='complete';try{localStorage.setItem(META,'1')}catch{}save();el.ending.hidden=true;document.body.classList.remove('ending-mode','ending-black');el.game.hidden=true;el.title.classList.add('active');endingPetalStage=0;seedPetals()}
}
